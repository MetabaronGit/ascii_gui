...
class Player:
    def __init__(self, name: str, hp_max, combat, munition, command, credits, tab_lines):
        self.__name = name
...
def draw_card(card):
    text = f"=< {card.get_name()} >" + "=" * (SCREEN_WIDTH_PX // CONSOLE_FONT_WIDTH_PX - len(card.get_name()))
    y = 0
...
def draw_cursor(actual_action, options: list):
...
while not game_over:
    for event in pygame.event.get():
        if event.type == pygame.KEYUP:
            ...
            screen.fill((0, 0, 0))
            draw_card(card1)
            draw_cursor(actual_action, [1,2,3,4,5,6])
            draw_console(player)
